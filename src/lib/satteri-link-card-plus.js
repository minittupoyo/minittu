...
const cacheDirectoryName = 'remark-link-card-plus';
const buildRenderKey = process.env.ASTRO_COMMAND === 'build' ? Date.now().toString(36) : 'dev';

/**
 * Create a Satteri-compatible copy of remark-link-card-plus.
 *
 * It converts a paragraph containing only a bare URL into the same class-based
 * link-card HTML shape used by remark-link-card-plus.
 */
...
async function cacheAsset(assetUrl, timeoutMs, fallbackUrl = assetUrl) {
  ...
  try {
    ...
    if (cachedFile) {
      ...
      return `/remark-link-card-plus/${cachedFile}`;
    }
    ...
    return `/remark-link-card-plus/${fileName}`;
  } catch {
    return fallbackUrl;
  }
}
...
function renderLinkCard(url, og, config) {
  ...
  const thumbnail = config.noThumbnail || !og.imageUrl
    ...
    : [
        ...
        `      <img src="${escapeHtml(og.imageUrl)}" class="remark-link-card-plus__image" alt="">`,
        '    </div>',
      ].join('\n');
  const favicon = config.noFavicon || !og.faviconUrl
    ? ''
    : `        <img src="${escapeHtml(og.faviconUrl)}" class="remark-link-card-plus__favicon" width="14" height="14" alt="">\n`;
  const main = [
    '    <div class="remark-link-card-plus__main">',
    '      <div class="remark-link-card-plus__content">',
    `        <div class="remark-link-card-plus__title">${escapeHtml(og.title)}</div>`,
    `        <div class="remark-link-card-plus__description">${escapeHtml(og.description)}</div>`,
    '      </div>',
    '      <div class="remark-link-card-plus__meta">',
    `${favicon}        <span class="remark-link-card-plus__url">${escapeHtml(displayedUrl)}</span>`,
    '      </div>',
    '    </div>',
  ].join('\n');
  ...
  return [
    ...
    `  <a href="${escapeHtml(url.href)}" target="_blank" rel="noreferrer noopener" class="remark-link-card-plus__card">`,
    body,
    '  </a>',
    '</div>',
  ].join('\n');
